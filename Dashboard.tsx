...
const defaultSections: Section[] = [
  {
    ...
    rows: [
      ...
      { Category: "Borrowed Money", Amount: "", Status: "Optional", Priority: "High", Notes: "Advances owed back (MyPay, SpotMe, etc.)." },
    ],
  },
  {
    key: "bills",
    ...
    rows: [
      { Bill: "Car note", "Due Date": "", Amount: "", Status: "Unpaid", Notes: "" },
      { Bill: "Phone", "Due Date": "", Amount: "", Status: "Unpaid", Notes: "" },
      { Bill: "Insurance", "Due Date": "", Amount: "", Status: "Unpaid", Notes: "" },
    ],
  },
 {
    key: "income",
  ...
  },
  {
    ...
    columns: ["Item", "Category", "Current Level", "Needed", "Urgency", "Status", "Buy Next", "Notes"],
    rows: [
  ...
  },
  {
    ...
    rows: [
      { Goal: "Emergency Fund", Category: "Money", Target: "500", Current: "", "Progress %": "", Priority: "High", "Next Step": "Save first amount", Notes: "" },
...
function DashboardHome({
  ...
  return (
    <section className="content">
      ...
      <div className="proof">
        <ProofCard label="Net Position" value={money(metrics.netPosition)} source="money" open={open} danger={metrics.netPosition < 0} />
...
function SectionPage({
  ...
  return (
    <section className="content">
      ...
      {section.key === "money" && <MoneySummary metrics={metrics} />}
      {section.key === "debt" && <DebtSummary section={section} metrics={metrics} />}
      {section.key === "savings" && <SavingsSummary metrics={metrics} />}
      {section.key === "inventory" && <InventorySummary section={section} metrics={metrics} />}
...
const BILL_STATUSES = ["Unpaid", "Paid", "Partial", "Overdue"] as const;

function MoneySummary({ metrics }: { metrics: Metrics }) {
  const rows = [
    ["Cash On Hand", money(metrics.cashOnHand)],
    ["Weekly Income", money(metrics.weeklyIncome)],
    ["Other Income", money(metrics.otherIncome)],
    ["Received Income", money(metrics.receivedIncome)],
    ["Food Needed", money(metrics.foodNeeded)],
    ["Gas Needed", money(metrics.gasNeeded)],
    ["Operating Cash", money(metrics.operatingCash)],
    ["Bills Pressure", money(metrics.billsPressure)],
    ["Debt Pressure", money(metrics.debtPressure)],
    ["Borrowed Money", money(metrics.borrowedMoney)],
    ["Spendable Cash", money(metrics.spendableCash)],
    ["Savings Vault", money(metrics.savingsVault)],
    ["Net Position", money(metrics.netPosition)],
  ];

  return (
    <div className="sectionSummary">
      <p className="kicker">LIVE_CALCULATIONS</p>
      <div className="summaryGrid">
        {rows.map(([label, value]) => (
          <div key={label} className={`summaryItem${label === "Spendable Cash" && metrics.spendableCash < 0 ? " danger" : ""}${label === "Net Position" && metrics.netPosition < 0 ? " danger" : ""}`}>
            <p>{label}</p>
            <h3>{value}</h3>
          </div>
        ))}
...
function DebtSummary({ section, metrics }: { section: Section; metrics: Metrics }) {
  const activeRows = section.rows.filter((row) => !["paid", "closed", "done"].includes((row.Status ?? "").toLowerCase()));
  const blocking = activeRows.filter((row) => debtBlocksCash(row) === "Yes").length;

  return (
    <div className="sectionSummary">
      <p className="kicker">DEBT_PRESSURE</p>
      <div className="summaryGrid">
        <div className="summaryItem"><p>Total Balance</p><h3>{money(metrics.totalDebtBalance)}</h3></div>
        <div className="summaryItem"><p>Payment Due</p><h3>{money(metrics.debtPressure)}</h3></div>
        <div className="summaryItem"><p>Active Debts</p><h3>{metrics.activeDebt}</h3></div>
        <div className="summaryItem"><p>Blocks Future Cash</p><h3>{blocking}</h3></div>
      </div>
    </div>
  );
}

function SavingsSummary({ metrics }: { metrics: Metrics }) {
  return (
    <div className="sectionSummary">
      <p className="kicker">VAULT_STATUS</p>
      <div className="summaryGrid">
        <div className="summaryItem"><p>Total Vault</p><h3>{money(metrics.savingsVault)}</h3></div>
        <div className="summaryItem"><p>Protected</p><h3>{money(metrics.protectedSavings)}</h3></div>
        <div className="summaryItem"><p>Allowed Pull</p><h3>{money(metrics.allowedWithdrawal)}</h3></div>
        <div className="summaryItem"><p>Locked</p><h3>{money(metrics.lockedSavings)}</h3></div>
      </div>
      <small className="summaryNote">Protected savings do not count as spendable cash unless Allowed Withdrawal is entered and status is approved.</small>
    </div>
  );
}

function InventorySummary({ section, metrics }: { section: Section; metrics: Metrics }) {
  const buyNext = section.rows
    .map((row) => ({ item: row.Item || "Item", next: getInventoryBuyNext(row) }))
    .filter((entry) => entry.next.startsWith("🛒") || entry.next.startsWith("Next:"))
    .slice(0, 4);

  return (
    <div className="sectionSummary">
      <p className="kicker">SURVIVAL_STOCK</p>
      <div className="summaryGrid">
        <div className="summaryItem"><p>Critical Items</p><h3>{metrics.criticalInventory}</h3></div>
        <div className="summaryItem"><p>Low Stock</p><h3>{metrics.lowInventory}</h3></div>
      </div>
      {buyNext.length > 0 && (
        <div className="summaryList">
          <p>BUY NEXT</p>
          {buyNext.map((entry) => (
            <span key={entry.item}>{entry.next}</span>
          ))}
        </div>
      )}
    </div>
...
function getDropdownOptions(sectionKey: SectionKey, column: string): string[] | null {
  if (sectionKey === "bills" && column === "Status") return [...BILL_STATUSES];
  if (sectionKey === "money" && column === "Status") return ["Needs update", "Confirmed", "Optional", "Watch"];
  if (sectionKey === "money" && column === "Priority") return ["Critical", "High", "Medium", "Low"];
  if (sectionKey === "income" && column === "Income Type") return ["Job", "Side", "Gift", "Refund", "Other"];
  if (sectionKey === "transactions" && column === "Type") return ["Expense", "Income", "Deposit"];
  if (sectionKey === "debt" && column === "Status") return ["Active", "Paid", "Closed"];
  if (sectionKey === "savings" && column === "Protected") return ["Yes", "No"];
  if (sectionKey === "savings" && column === "Vault Type") return ["Emergency", "Future", "Growth"];
  if (sectionKey === "savings" && column === "Status") return ["Locked", "Watch", "Approved", "Allowed", "Unlocked"];
  if (sectionKey === "inventory" && column === "Current Level") return ["Empty", "Low", "Medium", "High"];
  if (sectionKey === "inventory" && column === "Urgency") return ["Critical", "High", "Medium", "Low"];
  if (sectionKey === "inventory" && column === "Status") return ["Needed", "Watch", "Handled", "Done", "Bought"];
  if (sectionKey === "goals" && column === "Priority") return ["Critical", "High", "Medium", "Low"];
  if (sectionKey === "missions" && column === "Priority") return ["Critical", "High", "Medium", "Low"];
  if (sectionKey === "missions" && column === "Status") return ["Open", "In Progress", "Done", "Closed"];
  return null;
}

function isDateColumn(sectionKey: SectionKey, column: string) {
  if (column === "Due Date") return sectionKey === "bills" || sectionKey === "debt" || sectionKey === "missions";
  if (column === "Date Received") return sectionKey === "income";
  if (column === "Date") return sectionKey === "transactions";
  return false;
}

function isReadOnlyColumn(sectionKey: SectionKey, column: string) {
  if (sectionKey === "bills") return column === "Priority" || column === "Auto Alert";
  if (sectionKey === "inventory") return column === "Buy Next";
  return false;
}

function getReadOnlyValue(sectionKey: SectionKey, column: string, row: Row) {
  if (sectionKey === "bills" && column === "Priority") return getBillPriority(row);
  if (sectionKey === "bills" && column === "Auto Alert") return getBillAutoAlert(row);
  if (sectionKey === "inventory" && column === "Buy Next") return getInventoryBuyNext(row);
  return "";
}
...
function SpreadsheetGrid({
  ...
  const isBills = section.key === "bills";
  const hideActionHead = isBills;
      ...
      <table className="spreadsheetGrid" aria-label={`${section.label} spreadsheet`}>
        <thead>
          <tr>
            ...
            <th className="actionHead">{hideActionHead ? "" : "Action"}</th>
          </tr>
        </thead>
          ...
          {section.rows.map((row, rowIndex) => (
            <tr key={`${section.key}-${rowIndex}`}>
              {section.columns.map((column, columnIndex) => {
                const dropdownOptions = getDropdownOptions(section.key, column);
                const readOnly = isReadOnlyColumn(section.key, column);
                const isDate = isDateColumn(section.key, column);

                return (
                <td
                  key={column}
                  className={`spreadsheetCell${readOnly ? " readonlyCell" : ""}`}
                  onMouseDown={(event) => {
                    if (readOnly) return;
                    const input = event.currentTarget.querySelector<HTMLInputElement | HTMLSelectElement>("input, select");
                ...
                >
                  {readOnly ? (
                    <span className="spreadsheetReadonly" aria-label={`${section.label} row ${rowIndex + 1} ${column}`}>
                      {getReadOnlyValue(section.key, column, row)}
                    </span>
                  ) : dropdownOptions ? (
                    <select
                      className="spreadsheetInput spreadsheetSelect"
                      value={dropdownOptions.includes(row[column] ?? "") ? row[column] : dropdownOptions[0]}
...
function getMetrics(sections: Section[]): Metrics {
  ...
  const borrowedMoney = moneyAmount(moneySection, "Borrowed Money");
  ...
  const overdueBillRows = unpaidBills.filter((row) => isBillPastDue(row));
  const overdueBills = overdueBillRows.length;
  const transportOverdueBills = overdueBillRows.filter((row) => isTransportBill(row)).length;
  ...
  const spendableCash = operatingCash - totalPressure - borrowedMoney;
  const netPosition = spendableCash + allowedWithdrawal - totalDebtBalance;
  ...
  const lowInventory = inventorySection.rows.filter((row) => {
    const level = (row["Current Level"] ?? "").toLowerCase();
    const status = (row.Status ?? "").toLowerCase();
    return ["empty", "low"].includes(level) && !["done", "handled", "bought"].includes(status);
  }).length;
...
function getAlerts(metrics: Metrics, sections: Section[]): Alert[] {
  ...
  if (metrics.foodNeeded > 0 && metrics.spendableCash < metrics.foodNeeded) {
    alerts.push({
      title: "Food money is short",
      source: "money",
      level: "Critical",
      proof: `Food Needed ${money(metrics.foodNeeded)} · Spendable Cash ${money(metrics.spendableCash)}.`,
      action: "Cover food before wants, subscriptions, or nonessential bills.",
    });
  }

  if (metrics.gasNeeded > 0 && metrics.spendableCash < metrics.gasNeeded) {
    alerts.push({
      title: "Gas money threatens income",
      source: "money",
      level: "Critical",
      proof: `Gas Needed ${money(metrics.gasNeeded)} · Spendable Cash ${money(metrics.spendableCash)}.`,
      action: "Protect transportation before lower-priority overdue bills.",
    });
  }
  ...
  inventory.rows.forEach((row) => {
    ...
    const survival = ["food", "gas", "hygiene", "car", "house", "home"].some((tag) =>
      `${row.Item ?? ""} ${category}`.toLowerCase().includes(tag)
    );
    if (urgency === "critical" && !["done", "handled", "bought"].includes(status)) {
      alerts.push({
        title: `${row.Item || "Inventory item"} is critical`,
        source: "inventory",
        level: "Critical",
        proof: `${row.Item || "Item"} · ${row.Urgency} · ${getInventoryBuyNext(row)}.`,
        action: survival
          ? "Buy or cover this survival item before discretionary spending."
          : "Handle or update this before it becomes a bigger problem.",
      });
    } else if (urgency === "high" && !["done", "handled", "bought"].includes(status)) {
      alerts.push({
        title: `${row.Item || "Inventory item"} needs attention`,
        source: "inventory",
        level: "High",
        proof: `${row.Item || "Item"} is marked ${row.Urgency || "high"}.`,
        action: "Handle or update this before it becomes a bigger problem.",
      });
    }
  });

  if (metrics.transportOverdueBills > 0) {
    const transportNames = billsSection.rows
      .filter((row) => isBillPastDue(row) && isTransportBill(row))
      .map((row) => row.Bill || "Transport bill")
      .slice(0, 3)
      .join(", ");
    alerts.push({
      title: "Transport bills protect income",
      source: "bills",
      level: "Critical",
      proof: `${metrics.transportOverdueBills} transport bill(s) overdue: ${transportNames}.`,
      action: "Handle car, insurance, or phone before lower-priority overdue bills.",
    });
  }

  const shutoffOverdue = billsSection.rows.filter((row) => isBillPastDue(row) && isShutoffBill(row));
  if (shutoffOverdue.length > 0) {
    alerts.push({
      title: "Shutoff or shelter risk detected",
      source: "bills",
      level: "Critical",
      proof: `${shutoffOverdue.length} bill(s) tied to shelter or utilities are overdue.`,
      action: "Protect housing and utilities before wants or flexible debt.",
    });
  }

  const otherOverdue = metrics.overdueBills - metrics.transportOverdueBills - shutoffOverdue.length;
  if (otherOverdue > 0) {
    alerts.push({
      title: "Other overdue bills need review",
      source: "bills",
      level: "High",
      proof: `${otherOverdue} overdue bill(s) outside transport/shelter priority.`,
      action: "Review bills and pay or negotiate what you can after survival needs.",
    });
  }

  if (metrics.spendableCash < 0) {
    alerts.push({
      title: "Spendable Cash is negative",
      source: "money",
      level: "Critical",
      proof: `Operating ${money(metrics.operatingCash)} · Pressure ${money(metrics.totalPressure + metrics.borrowedMoney)} · Spendable ${money(metrics.spendableCash)}.`,
      action: "Cut lowest-priority pressure or find income before new spending.",
    });
  }

  if (metrics.unpaidBills > 0 && metrics.overdueBills === 0) {
    alerts.push({
      title: "Unpaid bills are creating pressure",
      source: "bills",
      level: metrics.unpaidBills >= 3 ? "High" : "Medium",
  ...
  if (metrics.activeDebt > 0 && metrics.debtPressure > 0) {
    alerts.push({
      title: "Active debt is eating future cash",
      source: "debt",
      level: metrics.debtPressure >= metrics.weeklyIncome && metrics.weeklyIncome > 0 ? "Critical" : "High",
      proof: `${metrics.activeDebt} active debt(s). Balance ${money(metrics.totalDebtBalance)} · Payment pressure ${money(metrics.debtPressure)}.`,
  ...
  if (metrics.borrowedMoney > 0) {
    alerts.push({
      title: "Borrowed money must be paid back",
      source: "money",
      level: "High",
      proof: `Borrowed/advances tracked: ${money(metrics.borrowedMoney)}.`,
      action: "Plan payback before treating cash as fully spendable.",
    });
  }
...
function getRecommendedMove(metrics: Metrics, alerts: Alert[]): RecommendedMove {
  if (metrics.cashOnHand === 0 && metrics.weeklyIncome === 0 && metrics.otherIncome === 0 && metrics.receivedIncome === 0) {
    return {
  ...
  if (metrics.foodNeeded > 0 && metrics.spendableCash < metrics.foodNeeded) {
    return {
      title: "Cover food before anything else",
      why: `Food Needed is ${money(metrics.foodNeeded)} but Spendable Cash is only ${money(metrics.spendableCash)}.`,
      doFirst: "Open Money Snapshot and Inventory. Make sure food money is covered before wants or flexible bills.",
      doNotDo: "Do not spend on subscriptions, wants, or low-priority overdue bills first.",
      checkpoint: "Recheck Spendable Cash after food is covered or updated.",
      source: "money",
      tone: "danger",
    };
  }
  ...
  if (metrics.gasNeeded > 0 && metrics.spendableCash < metrics.gasNeeded) {
    return {
      title: "Protect gas and transportation",
      why: `Gas Needed is ${money(metrics.gasNeeded)}. Without transport, income is at risk.`,
      doFirst: "Cover gas before lower-priority overdue bills that do not protect income.",
      doNotDo: "Do not let car, insurance, or phone lapse while chasing nonessential pressure.",
      checkpoint: "Update Gas Needed and Inventory once covered.",
      source: "money",
      tone: "danger",
    };
  }
  ...
  if (metrics.criticalInventory > 0) {
    return {
      title: "Handle survival inventory first",
      why: `${metrics.criticalInventory} inventory item(s) are critical or high urgency.`,
      doFirst: "Open Inventory and cover food, gas, hygiene, car, or home needs first.",
  ...
  if (metrics.transportOverdueBills > 0) {
    return {
      title: "Handle transport bills before other overdue bills",
      why: `${metrics.transportOverdueBills} car, insurance, or phone bill(s) are overdue. That threatens income and mobility.`,
      doFirst: "Open Bills and handle transport-related overdue items before lower-priority overdue bills.",
      doNotDo: "Do not pay discretionary bills while car, insurance, or phone are at risk.",
      checkpoint: "Update bill statuses after transport bills are handled or negotiated.",
      source: "bills",
      tone: "danger",
    };
  }
  ...
  if (metrics.spendableCash < 0 && metrics.protectedSavings > 0) {
    return {
      ...
      why: `Spendable Cash is ${money(metrics.spendableCash)}, but savings is locked. Pressure must be managed from operating cash first.`,
  ...
  if (metrics.overdueBills > 0) {
    return {
      title: "Review remaining overdue bills",
      why: `${metrics.overdueBills} bill(s) are overdue or past due after survival needs are checked.`,
  ...
  if (metrics.debtPressure > 0 && metrics.activeDebt > 0) {
    return {
      title: "Attack debt blocking the next check",
      why: `${metrics.activeDebt} active debt(s) with ${money(metrics.debtPressure)} in payment pressure.`,
      doFirst: "Open Debt and update balances, due dates, and the highest-pressure item.",
      doNotDo: "Do not ignore debt that will hit before the next income.",
      checkpoint: "Recheck Debt Pressure after one payment plan is set.",
      source: "debt",
      tone: "warning",
    };
  }
...
function cardFor(key: SectionKey, metrics: Metrics, alerts: Alert[]) {
  switch (key) {
    ...
    case "income":
      return {
        value: money(metrics.weeklyIncome + metrics.otherIncome + metrics.receivedIncome),
        label: "Total income tracked",
        detail: `Snapshot ${money(metrics.weeklyIncome + metrics.otherIncome)} · Received ${money(metrics.receivedIncome)}`,
        tone: "good",
      };
    case "transactions":
      return {
    ...
    case "debt":
      return {
        ...
        detail: `${metrics.activeDebt} active · Balance ${money(metrics.totalDebtBalance)}`,
    ...
    case "inventory":
      return {
        ...
        detail: `${metrics.lowInventory} low stock`,
        tone: metrics.criticalInventory > 0 ? "warn" : metrics.lowInventory > 0 ? "neutral" : "good",
      };
    case "goals":
      return {
...
function billLabel(row: Row) {
  return `${row.Bill ?? ""}`.toLowerCase();
}

function isTransportBill(row: Row) {
  const name = billLabel(row);
  return ["car", "auto", "insurance", "phone", "transport", "gas"].some((tag) => name.includes(tag));
}

function isShutoffBill(row: Row) {
  const name = billLabel(row);
  return ["electric", "power", "water", "rent", "mortgage", "utility", "utilities", "internet"].some((tag) =>
    name.includes(tag)
  );
}

function getInventoryBuyNext(row: Row) {
  const status = (row.Status ?? "").toLowerCase();
  if (["done", "handled", "bought"].includes(status)) return "—";
  const level = (row["Current Level"] ?? "").toLowerCase();
  const urgency = (row.Urgency ?? "").toLowerCase();
  ...
  if (urgency === "critical" || level === "empty") return survival ? `🛒 ${item} NOW` : `🛒 ${item} NOW`;
  if (urgency === "high" || level === "low") return `Next: ${item}`;
  if (level === "medium") return "Watch";
  return "—";
}

function debtBlocksCash(row: Row) {
  const status = (row.Status ?? "").toLowerCase();
  if (["paid", "closed", "done"].includes(status)) return "No";
  const payment = number(row["Payment Due"]);
  const balance = number(row["Current Balance"]);
  if (payment > 0 || balance > 0) return "Yes";
  return "No";
}

function daysUntilDue(dueDate: string | undefined) {
...
function isBillPastDue(row: Row) {
  const status = (row.Status ?? "").toLowerCase();
  if (status === "paid") return false;
  return status === "overdue" || isOverdue(row["Due Date"]);
}

function getBillPriority(row: Row) {
  const status = (row.Status ?? "").toLowerCase();
  if (status === "paid") return "Paid";
  if (status === "partial") return "High";
  if (isBillPastDue(row)) return "Critical";
  const days = daysUntilDue(row["Due Date"]);
  if (days !== null && days <= 3) return "High";
  if (days !== null && days <= 7) return "Medium";
  return "Low";
}

function getBillAutoAlert(row: Row) {
  const status = (row.Status ?? "").toLowerCase();
  if (status === "paid") return "✅ Paid";
  if (status === "partial") return "🟡 Partial";
  if (isBillPastDue(row)) return "🚨 Overdue";
  const days = daysUntilDue(row["Due Date"]);
...
function focusSpreadsheetCell(sectionKey: SectionKey, rowIndex: number, columnIndex: number) {
  ...
  const cells = Array.from(
    document.querySelectorAll<HTMLInputElement | HTMLSelectElement>(
      `input[data-section="${sectionKey}"], select[data-section="${sectionKey}"]`
    )
  );
...
.spreadsheetGrid tr:nth-child(even) .readonlyCell {
...
/* ===== RESPONSIVE SPREADSHEET SCALING ===== */
